...
- DevSpace MCP；
- T3 Code；
- 为每个启用服务临时创建的 Cloudflare Quick Tunnel HTTPS/WSS 入口。
...
Quick Tunnel 不需要 Cloudflare 账户凭证。Fork 中的 workflow 只能使用 fork 自己配置的凭证。来自 fork 的 pull request 也不会因为本仓库的源码而自动获得上游 Secrets。本项目的特权 workflow 只使用 `workflow_dispatch`，不会自动执行 fork PR 中的代码。
...
Quick Tunnel 不需要 Cloudflare API token、DNS Edit 权限、自定义 hostname 或长期 connector credential。不要为该 workflow 增加此类 Cloudflare 凭证。

## T3 Code、DevSpace 与 Quick Tunnel 风险
...
- 随机 `trycloudflare.com` URL 是公网可达入口，不能依赖 URL 隐蔽性；
- DevSpace 访问控制依赖 OAuth Owner Token；
- T3 Code 访问控制依赖 pairing/session authentication；
- npm 包及其依赖会在 runner 上执行安装代码。

因此只应把 MCP、T3 和 coding-agent 连接授权给可信客户端。不要把 connection files、Owner Token、T3 pairing URL、T3/DevSpace 日志或 cloudflared 日志上传为 artifact 或粘贴到公开 issue/PR。

每个启用服务使用独立 Quick Tunnel 进程和独立随机 URL。这样 T3 Code 与 DevSpace 不共享 HTTP/WebSocket/OAuth 根路径，也允许同一 opaque target 同时启动多个完全隔离的 runner session。

Quick Tunnel URL 每次运行都会变化。Runner 或 cloudflared 进程结束后，该 URL 不再连接到本地服务。客户端中保存的旧连接会失效，必须使用当前 session 的私有 connection file 更新。

Quick Tunnel 是便捷的临时公网入口，不提供稳定地址、Cloudflare Access 策略绑定或账户级资源管理。应用层认证仍是必要控制。
...
- Quick Tunnel URL 和完整 cloudflared 日志；
- Headscale URL、auth key 和内部地址；
- 目标私有仓库名称；
- Git credential 文件；
- 完整 Tailscale status；
- DevSpace、T3 Code 和连接诊断日志。

正常 finalization 会终止相关进程并删除 connection files、Quick Tunnel URL files、共享 workspace、T3 state 和 Git credential。GitHub 直接销毁虚拟机时，即使 finalizer 未执行，所有本地文件和进程仍会随机器消失。
...
[ ] workflow 不持有 Cloudflare 账户或长期 tunnel 凭证
[ ] 每个启用服务使用独立 Quick Tunnel 进程
[ ] Quick Tunnel URL 和应用 credential 不进入 Actions 输出或 artifact
[ ] Headscale grants 不允许 runner 主动横向访问
[ ] DevSpace/T3/cloudflared/Tailscale 版本和校验值已经复核
```
