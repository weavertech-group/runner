...
| Staging deployment and health | **Passed** | Worker version `d6997cf5-7d00-4b1b-8280-6a3fbe58d1a8` is deployed at the configured Workers.dev origin with the OAuth KV binding, task Durable Object, and three GitHub App Worker secrets. `/health` returned exact body `ok`; OAuth metadata endpoints returned 200; unauthenticated `/mcp` returned 401 with a bearer challenge; invalid authorization and callback requests returned safe 400 responses. The runner repository `TASK_CONTROL_PLANE_URL` variable exactly matches the Worker origin. |
